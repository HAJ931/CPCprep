...
class Solution {
private:
    void inOrderTraversal(struct Node* root, struct Node** prev, struct Node** left, struct Node** mid, struct Node** right){
        if(!root)
            return;
        inOrderTraversal(root->left, prev, left, mid, right);
        ...
        *prev = root;
        inOrderTraversal(root->right, prev, left, mid, right);
    }
public:
    void correctBST( struct Node* root )
    {
        ...
        if(!root)
            return;
        struct Node* prev = NULL, *left = NULL, *mid = NULL, *right = NULL;
        inOrderTraversal(root, &prev, &left, &mid, &right);
        if(left && right){
            int temp = left->data;
